    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in" @click.self="$emit('close')">
      ...
      <div class="relative card w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div class="px-6 py-4 border-b flex items-center justify-between bg-muted/30">
          <div>
            <h2 class="text-lg font-semibold tracking-tight">Modifier la candidature</h2>
            <p class="text-xs text-muted-foreground">Mettez à jour les informations de l'opportunité.</p>
          </div>
          <div class="flex items-center gap-2">
            <button @click="$emit('delete', application.id)" class="p-2 hover:bg-destructive/10 text-muted-foreground hover:text-destructive rounded-full transition-colors" title="Supprimer">
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <form @submit.prevent="handleSubmit" id="edit-app-form" class="space-y-6">
            <!-- URL -->
            <div class="space-y-2">
            ...
            <div class="space-y-2">
              ...
              <p class="text-[10px] text-muted-foreground uppercase tracking-wider font-bold">Séparer par des virgules</p>
            </div>
...
const form = reactive({
  ...
  company_info: props.application.company_info || '',
  job_profile: props.application.job_profile || '',
  main_missions: props.application.main_missions || '',
  primary_skills: props.application.primary_skills || '',
  proposed_salary: props.application.proposed_salary || '',
  applied_at: props.application.applied_at
    ? new Date(props.application.applied_at).toISOString().split('T')[0]
    : '',
  company_feedback: props.application.company_feedback || '',
